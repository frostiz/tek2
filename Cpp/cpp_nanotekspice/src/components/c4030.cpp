...
nts::c4030::c4030(const std::string &value)
	: _truthtable {
	{nts::Tristate::XUNDEFINED, nts::Tristate::XUNDEFINED, nts::Tristate::XFALSE},
...
nts::Tristate		nts::c4030::gate_xor(const nts::Tristate &left, const nts::Tristate &right)
{
	for (int i = 0; i < 9; i++) {
...
nts::Tristate		nts::c4030::compute(size_t pin)
{
	...
	return this->gate_xor(result1, result2);
}

void			nts::c4030::setLink(size_t pin, nts::IComponent &other, size_t otherPin)
{
...
const size_t		&nts::c4030::getPinNumber() const {
	return _pinNumber;
}

const std::string 	&nts::c4030::getType() const {
	return _type;
}